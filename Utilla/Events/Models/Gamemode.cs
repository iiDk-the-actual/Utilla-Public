...
{
    /// <summary>
    ///     There is no gamemode manager to rely on, this should only be used by the Utilla mod when preparing modded gamemodes
    ///     or gamemodes using a unique gamemode manager.
    /// </summary>
    None,
...
public class Gamemode
{
    ...
    public Gamemode(string id, string displayName, GameModeType? game_mode_type = null)
    {
        ...
        ID = game_mode_type.HasValue && !id.EndsWith(game_mode_type.Value.ToString())
                     ? string.Concat(id, game_mode_type)
                     : id;

        DisplayName = displayName;

        Logging.Message(
                $"Constructed custom gamemode: {id} based on {(game_mode_type.HasValue ? game_mode_type.Value : "no")} type");
    }
